                                                                        #RoI Head，负责对 RPN 找出的区域进行“精修”（分类、回归坐标）以及“分割”（生成 Mask）
                                                            #原理：这是最忙碌的部分。它接收 Proposas，利用 RoIAlign 技术从特征图上抠出对应的特征小块。
"""
            它做三件事：
                分类：是什么物体？
                回归：框得再准一点。
                Mask：在框里通过全卷积网络 (FCN) 预测每个像素是前景还是背景。
"""
            #代码看点：maskrcnn_loss 函数计算分割损失；maskrcnn_inference 函数在预测时将 Mask 概率映射回原图
