                                                    #使用 ResNet50 作为主干网络提取特征，并结合了 FPN (Feature Pyramid Network) 来提升对小目标的检测能力
            #原理：ResNet50 负责提取图片特征。FPN（特征金字塔）则将高层的语义特征和底层的细节特征融合
#"D:\MASKRCNN_daima\models\LEGNet\mmrotate\models\backbones\legnet.py"(核心)：这是唯一对你有用的文件。里面定义了 LEGNet 的网络结构（卷积怎么连、注意力机制怎么算）。我们会把这个文件提取出来改造。
"""
要将骨干网络替换为 LEG，请专注于 backbone/resnet50_fpn_model.py 中的 resnet50_fpn_backbone 函数。
